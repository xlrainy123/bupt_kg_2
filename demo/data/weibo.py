...
class Weibo:
    cookie = {
        "Cookie": 'SUB=_2A25xD7leDeRhGeBG6FAU-S3Iyz-IHXVS88cWrDV6PUJbkdANLRnukW1NRg0GuxBE7nhGUIFyaWprXHeUgRPTwKAN; SUHB=0h1J73csu0CLCS; SCF=An4qs1Vf3I7EPiars1HvEpFDil3QE3JZUgkP-z-qsCp9pjvzpnF7Dlv1QvtnXkZc_0zj1SJpigEmcbx9iOj_Ues.; SSOLoginState=1544276238'
    ...
    def get_username(self):
        try:
            ...
            img_url = selector.xpath("//body/div[4]/img/@src")
            print('img_url:', img_url[0])
            self.info_dict['img_url'] = img_url[0]
            infos = selector.xpath("//body/div[7]/text()")
            print(infos)
            for info in infos:
                info = str(info)
                if '昵称' in info:
            ...
            labels = selector.xpath("//body/div[7]/a/text()")
            print(labels)
    ...
    def write_mysql(self):
        try:
            ...
            user_profile = user_profile_template.format(self.username, self.user_id, self.weibo_num, self.following, self.followers)
            try:
                print(user_profile)
            ...
            for i in range(1, self.weibo_num2 + 1):
                user_content = user_content_template.format(self.username, self.user_id, self.weibo_content[i - 1],
                                                            self.weibo_place[i - 1], self.publish_time[i - 1],self.up_num[i - 1],
                                                            self.retweet_num[i - 1], self.comment_num[i - 1], self.publish_tool[i - 1])
                try:
                    print(user_content)
...
ids = [6212090268,5427178881,3966868064,5599920821,5754775462,5817588052,5888917815,5683508732,6126500558,3199996080,
       6266052686,6011644949,6361326368]
